...
| T-22 | Cấp tài khoản đăng nhập cho nhân viên + gán vai trò theo chi nhánh (`user_branch_roles`) | Claude | DONE | Claude 12/09 | Trang `/admin/employees/{id}/account`: cấp tài khoản mới (bcrypt), đổi vai trò + chi nhánh, đặt lại mật khẩu, khoá/mở. Mật khẩu chỉ đi form → action → bcrypt, không vào log lẫn nhật ký. Đã xem trên bản triển khai với hồ sơ Hương; **không bấm thử nút khoá** vì đó là tài khoản của chính anh Khôi |
| T-30 | Nhập nhân viên từ tệp KiotViet | Claude | DONE | Claude 12/09 | **Đã nhập thật** trên bản triển khai — app đọc thẳng phương ngữ .xlsx thứ hai trong trình duyệt. NV000001 từ "Chủ spa" thành **Hương** kèm số điện thoại và hai chi nhánh; liên kết `user_id` tới tài khoản chủ spa còn nguyên. Đã bỏ hồ sơ "Lễ tân demo" để danh sách khớp đúng bản xuất (giữ tài khoản `letan@spa.local` để kiểm chứng phân quyền) |
...
| T-31 | Lọc lưới lịch theo KTV và phòng | Claude | DONE | Claude 12/09 | Ô lọc chỉ hiện khi có từ 2 lựa chọn trở lên (spa hiện 1 nhân viên, 0 phòng nên chưa thấy). Dòng tổng ghi rõ "lọc từ N"; rỗng do lọc nói khác rỗng do không có lịch |
| T-36 | Lịch hẹn định kỳ (lặp hằng tuần/tháng) | Claude | TODO | | Đặt một lịch lặp sinh đủ các buổi, huỷ một buổi không ảnh hưởng các buổi khác |
...
| T-34 | Huỷ hoá đơn có bút toán ngược: hoàn buổi về gói, đảo phiếu thu | Claude | DONE | Claude 12/09 | Migration 0013 gắn khoá ngoại `RESTRICT` còn thiếu, nên **xoá hoá đơn đã trừ buổi giờ bị chặn thẳng**. `db:check-invoices` lên **23/23**, thêm 2 ca: huỷ thì buổi về gói và số dư quỹ về như cũ, mà sổ cái vẫn còn cả `use` lẫn `adjust`. Đã huỷ thật HD000001 qua giao diện: sổ quỹ giữ cả `TTHD000001` thu lẫn `HTTTHD000001` chi |
